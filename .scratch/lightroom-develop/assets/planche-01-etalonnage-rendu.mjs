// Planche RENDU du ticket 01 (etalonnage) : rendu de l'effet sur DEUX photos
// d'Antoine par le pipeline REEL (iframe Vite 1421, module map vierge -> sert
// etalonnage.ts tel qu'il est sur disque, non commite compris). Onze colonnes,
// une par reglage a comparer contre le meme panneau dans Lightroom.
// Par ligne : vignette (~620, par masses) + un crop 1:1 (256 natif) sur la peau /
// la fleur, la ou un etalonnage se juge. Orientation EXIF respectee (from-image ;
// dims verifiees = source). Ecrit etal-{A,B}-col<N>-{vign,crop}.png + etal-stats.json.
// Usage : node planche-01-etalonnage-rendu.mjs   (app sur CDP 9223 + Vite worktree 1421)
...
// la comparaison cote a cote se fasse a reglage identique.
const COLS = [
  { key: "temoin", label: "Temoin (tout a 0)", temoin: true },
  { key: "rouge-teinte-moins", label: "Rouge teinte -60", params: { redHue: -60 } },
  { key: "rouge-teinte-plus", label: "Rouge teinte +60", params: { redHue: 60 } },
  { key: "vert-teinte-moins", label: "Vert teinte -60", params: { greenHue: -60 } },
  { key: "vert-teinte-plus", label: "Vert teinte +60", params: { greenHue: 60 } },
  { key: "bleu-teinte-moins", label: "Bleu teinte -60", params: { blueHue: -60 } },
  { key: "bleu-teinte-plus", label: "Bleu teinte +60", params: { blueHue: 60 } },
  { key: "bleu-sat-plus", label: "Bleu saturation +60", params: { blueSaturation: 60 } },
  { key: "nuance-moins", label: "Nuance foncee -80 (vert)", params: { shadowTint: -80 } },
  { key: "nuance-plus", label: "Nuance foncee +80 (magenta)", params: { shadowTint: 80 } },
  { key: "teal-orange", label: "Teal & orange (Bleu t-40 s+30, Rouge t+10)", params: { blueHue: -40, blueSaturation: 30, redHue: 10 } },
];
...
const FRAME_ID = "__planche01Etal";
...
for (const [tag, ph] of Object.entries(PHOTOS)) {
  ...
  const setup = JSON.parse(await evalIn(`(async () => {
    ...
    window.__etalSetup = { openDocument, r, W: bmp.width, H: bmp.height };
  ...
  const perCol = async (col) => {
    const c = ph.crop;
    const expr = `(async () => {
      const { openDocument, r } = window.__etalSetup;
      const col = ${JSON.stringify(col)};
      const stack = openDocument(r, "fond").stack;
      if (!col.temoin) {
        const a = stack.addLayer("etalonnage");
  ...
  for (let i = 0; i < COLS.length; i++) {
    ...
    writeFileSync(path.join(OUT, `etal-${tag}-col${i}-vign.png`), vb);
    writeFileSync(path.join(OUT, `etal-${tag}-col${i}-crop.png`), cb);
    ...
    console.log(`${tag} col${i} ${col.key.padEnd(20)} mean=${res.mean.toFixed(2)} std=${res.std.toFixed(2)} md5v=${md5v.slice(0,8)}`);
  }
  ...
  await evalIn(`delete window.__etalSetup; "ok"`, frameCtx.id);
}

writeFileSync(path.join(OUT, "etal-stats.json"), JSON.stringify(stats, null, 2));
console.log("\nstats -> etal-stats.json");
