// Planche RENDU du chantier 02b (parite Lightroom : voile global, WB sans
// renormalisation, blancs/noirs locaux, clarte neutre en couleur) sur DEUX
// photos d'Antoine, pipeline REEL (iframe Vite 1421, CDP 9223).
// Usage : node planche-02b-parite-rendu.mjs
...
const COLS = [
  ...
  { key: "temp-p100", label: "Temperature +100 (rechauffe, eclaircit)", params: { temperature: 100 } },
  { key: "temp-m100", label: "Temperature -100 (refroidit, eclaircit)", params: { temperature: -100 } },
  { key: "blancs-p100", label: "Blancs +100 (local)", params: { whites: 100 } },
  { key: "noirs-m100", label: "Noirs -100 (local)", params: { blacks: -100 } },
  { key: "clarte-p100", label: "Clarte +100 (couleurs tenues)", params: { clarity: 100 } },
];
...
for (const [tag, ph] of Object.entries(PHOTOS)) {
  ...
  for (let i = 0; i < COLS.length; i++) {
    ...
    writeFileSync(path.join(OUT, `parite02b-${tag}-col${i}-vign.png`), vb);
    writeFileSync(path.join(OUT, `parite02b-${tag}-col${i}-crop.png`), cb);
...
writeFileSync(path.join(OUT, "parite02b-stats.json"), JSON.stringify(stats, null, 2));
console.log("\nstats -> parite02b-stats.json");
