// Planche RENDU du ticket 05 (module hsl de l'etage) : le HSL rendu sur DEUX
// photos d'Antoine par le pipeline REEL (iframe Vite 1421, module map vierge ->
// sert le disque non commite compris). Le module vit dans l'ETAGE, donc on le
// pilote par r.setDevelop({ hsl: params }) + exportFrame(layers, null, develop) —
// PAS un calque. Une colonne par reglage a comparer au meme panneau (TSL /
// Couleur / N&B) dans Lightroom. Par ligne : vignette (~620, par masses, ou le
// HSL se juge) + crop 1:1 (256 natif) sur la peau (Saturation de l'orange).
// Ecrit hsl-{A,B}-col<N>-{vign,crop}.png + hsl-stats.json.
// Usage : node planche-05-hsl-rendu.mjs   (app sur CDP 9223 + Vite worktree 1421)
...
const COLS = [
  ...
  { key: "teinte-rouge-p", label: "Teinte rouge +100", params: { redHue: 100 } },
  { key: "teinte-rouge-m", label: "Teinte rouge -100", params: { redHue: -100 } },
  { key: "teinte-bleu-p", label: "Teinte bleu +100", params: { blueHue: 100 } },
  { key: "teinte-bleu-m", label: "Teinte bleu -100", params: { blueHue: -100 } },
  { key: "sat-orange-p", label: "Saturation orange +100 (peau)", params: { orangeSat: 100 } },
  { key: "sat-bleu-m", label: "Saturation bleu -100", params: { blueSat: -100 } },
  { key: "lum-vert-p", label: "Luminance vert +100", params: { greenLum: 100 } },
  { key: "lum-vert-m", label: "Luminance vert -100", params: { greenLum: -100 } },
  { key: "nb-neutre", label: "Noir et blanc (neutre)", params: { mode: 1 } },
  { key: "nb-rouge-bleu", label: "N&B : rouge +100 / bleu -100", params: { mode: 1, redGray: 100, blueGray: -100 } },
  { key: "nb-orange", label: "N&B : orange +80", params: { mode: 1, orangeGray: 80 } },
];
...
for (const [tag, ph] of Object.entries(PHOTOS)) {
  ...
  const setup = JSON.parse(await evalIn(`(async () => {
    ...
    window.__hslSetup = { openDocument, r, W: bmp.width, H: bmp.height };
  ...
  const perCol = async (col) => {
    const c = ph.crop;
    const expr = `(async () => {
      const { openDocument, r } = window.__hslSetup;
      const col = ${JSON.stringify(col)};
      const stack = openDocument(r, "fond").stack;
      const develop = col.temoin ? {} : { hsl: col.params };
  ...
  for (let i = 0; i < COLS.length; i++) {
    ...
    writeFileSync(path.join(OUT, `hsl-${tag}-col${i}-vign.png`), vb);
    writeFileSync(path.join(OUT, `hsl-${tag}-col${i}-crop.png`), cb);
  ...
  await evalIn(`delete window.__hslSetup; "ok"`, frameCtx.id);
}

writeFileSync(path.join(OUT, "hsl-stats.json"), JSON.stringify(stats, null, 2));
console.log("\nstats -> hsl-stats.json");
