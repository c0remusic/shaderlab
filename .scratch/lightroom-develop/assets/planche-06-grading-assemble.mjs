// Planche ASSEMBLE du ticket 06 (module colorGrading) : construit planche-06-grading.html a
// partir des PNG et de grading-stats.json ecrits par planche-06-grading-rendu.mjs. HTML et
// PNG ne sont PAS versionnes (regenerables) ; ce script l'est.
// Usage : node planche-06-grading-assemble.mjs
import { readFileSync, writeFileSync } from "node:fs";
import path from "node:path";

const OUT = path.dirname(new URL(import.meta.url).pathname.replace(/^\/([A-Za-z]:)/, "$1"));
const stats = JSON.parse(readFileSync(path.join(OUT, "grading-stats.json"), "utf-8"));
...
for (const [tag, ph] of Object.entries(stats.photos)) {
  ...
  for (const col of ph.cols) {
    body += `<div class="col">
      ...
      <img class="vign" src="grading-${tag}-col${col.i}-vign.png" alt="${esc(col.key)}">
      <img class="crop" src="grading-${tag}-col${col.i}-crop.png" alt="${esc(col.key)} crop">
...
<title>Planche 06 — Color Grading (roues)</title>
<style>
...
writeFileSync(path.join(OUT, "planche-06-grading.html"), html);
console.log("ecrit planche-06-grading.html");
