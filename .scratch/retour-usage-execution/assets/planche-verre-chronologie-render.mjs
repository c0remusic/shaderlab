// Planche chronologie du VERRE — rend Poli et Depoli pour UN jalon.
//   Antoine : "je preferais des versions precedentes du verre je crois".
//   Verdict flou -> planche chronologique, il pointe la colonne preferee.
//
// glass.ts est suppose DEJA swappe sur disque par le shell (git show <sha>:...)
// AVANT d'appeler ce script. Une invocation = un tag = un iframe FRAIS ->
// modules relus depuis le DISQUE (render-check-page.html, module map vierge).
//
// Scene : vram-test/photo-1.jpg (meme photo que la planche q2). Matieres
// Poli (6) et Depoli (7), indices stables sur tous les jalons (verifie).
// Params EXPLICITES identiques d'un jalon a l'autre (defauts identiques sur
// tous les jalons, verifie ; on les pose quand meme en clair pour l'apples-to-
// apples). Vignette reduite (voile par masses) + crop 1:1 fixe sur le bord le
// plus contraste (micro-relief par pixel). Crop deterministe : meme photo
// source -> meme coordonnee a chaque invocation.
//
// Usage : node planche-verre-chronologie-render.mjs <tag>
...
if (!TAG) throw new Error("usage: node planche-verre-chronologie-render.mjs <tag>");
...
const FRAME_ID = "__plancheChronoFrame_" + TAG;
...
// Poli=6, Depoli=7. Bundle de params EXPLICITES (valeurs = defauts communs a
// tous les jalons ; poses en clair pour garantir l'apples-to-apples).
const COMMON = { dispersion: 0.25, diffusion: 0.08, depth: 0.5, grain: 0.3, specular: 0.35, relief: 1, thickness: 0.35 };
const MATIERES = [
  { id: "poli", material: 6 },
...
const script = `(async () => {
  ...
  // CROP fixe = bord le plus contraste de la PHOTO source (deterministe, meme
  // point pour tous les jalons puisque meme photo). Gradient de luminance.
  const S = 128;
  ...
  for (const mat of MATIERES) {
    ...
    stack.updateParams(lid, Object.assign({ material: mat.material }, COMMON));
    const frame = await renderer.exportFrame(stack.layers);
    out[mat.id] = {
      img: versDataUrl(frame),
      crop: cropDataUrl(frame, bx, by),
      stat: stat(frame),
    };
  }
...
for (const [k, o] of Object.entries(res)) {
  if (k === "crop") continue;
  const imgBuf = Buffer.from(o.img.split(",")[1], "base64");
  const cropBuf = Buffer.from(o.crop.split(",")[1], "base64");
  writeFileSync(path.join(OUT, `chrono-${TAG}-${k}.png`), imgBuf);
  writeFileSync(path.join(OUT, `chrono-${TAG}-${k}-crop.png`), cropBuf);
  const md5 = (b) => createHash("md5").update(b).digest("hex");
  manifest[k] = { ...o.stat, md5: md5(imgBuf), md5crop: md5(cropBuf) };
  console.log(`${TAG} ${k.padEnd(7)} moyenne ${String(o.stat.moyenne).padStart(6)}  ecart-type ${String(o.stat.ecartType).padStart(6)}  md5 ${manifest[k].md5.slice(0, 8)}  md5crop ${manifest[k].md5crop.slice(0, 8)}`);
}
writeFileSync(path.join(OUT, `chrono-manifest-${TAG}.json`), JSON.stringify(manifest, null, 2));
console.log(`crop fixe @ (${res.crop.x}, ${res.crop.y})`);
