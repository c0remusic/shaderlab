// Planche de MECANISME 5 ticket 06 (aquarelle v4 REECRIT, aquarelle.ts sur disque)
// rendu sur DEUX photos d'Antoine par le pipeline REEL (iframe Vite 1421, module map
// vierge : sert aquarelle.ts tel qu'il est). Huit colonnes rendues ici (0..7) ; la
// colonne 8 (rappel v3 planche 4 col6) est ajoutee par l'assembleur a partir des PNG
// aq4-*. Deux crops 1:1 (256 natif) par photo, memes coords que la planche 4.
// Orientation EXIF respectee (createImageBitmap from-image ; dims verifiees = source).
// Ecrit aq5-{A,B}-col<N>-{vign,crop1,crop2}.png + aq5-stats.json.
// Ne touche a aucun aq-/aq2-/aq3-/aq4-*.
// Usage : node planche-06-rendu-5.mjs   (app sur CDP 9222 + Vite worktree sur 1421)
...
// Defauts v4 (tout le fichier). Les tailles sont RELATIVES a la largeur :
// washRadius/frontWidth/fibers en fraction, spread unitless.
const FULL = {
  wash: 0.85, washRadius: 0.005, spread: 2.0, wetness: 0.7, fibers: 0.003,
  edgeDarkening: 0.9, frontWidth: 0.003, lace: 0.6, bloom: 0.15, paper: 0.5,
};

// 0 temoin ; 1-3 tout v4 papier 0/defaut/1 ; 4 lavis seul ; 5 sans front ;
// 6 bave x2 ; 7 sans lavis. (col 8 = rappel v3, ajoute par l'assembleur.)
const COLS = [
  { key: "temoin", temoin: true },
  { key: "papier0",   params: { ...FULL, paper: 0 } },
  { key: "papierdef", params: { ...FULL } },
  { key: "papier1",   params: { ...FULL, paper: 1 } },
  { key: "lavis",     params: { ...FULL, wetness: 0, fibers: 0, edgeDarkening: 0 } },
  { key: "sansfront", params: { ...FULL, edgeDarkening: 0 } },
  { key: "bave2x",    params: { ...FULL, spread: 4.0 } },
  { key: "sanslavis", params: { ...FULL, wash: 0 } },
];
...
const FRAME_ID = "__planche06Frame5";
...
const injectSetup = async () => {
  await evalIn(`(async () => { if (!window.__aqSetup5) {
    ...
    await r.loadImage(bmp); window.__aqSetup5 = { openDocument, r, W: bmp.width, H: bmp.height };
...
for (const [tag, ph] of Object.entries(PHOTOS)) {
  ...
  const setup = JSON.parse(await evalIn(`(async () => {
    ...
    window.__aqSetup5 = { openDocument, r, W: bmp.width, H: bmp.height };
  ...
  const perCol = async (col) => {
    ...
    const expr = `(async () => {
    const { openDocument, r, W, H } = window.__aqSetup5;
  ...
  for (let i = 0; i < COLS.length; i++) {
    ...
    writeFileSync(path.join(OUT, `aq5-${tag}-col${i}-vign.png`), vb);
    writeFileSync(path.join(OUT, `aq5-${tag}-col${i}-crop1.png`), c1b);
    writeFileSync(path.join(OUT, `aq5-${tag}-col${i}-crop2.png`), c2b);
  ...
  await evalIn(`delete window.__aqSetup5; "ok"`, frameCtx.id);
}

writeFileSync(path.join(OUT, "aq5-stats.json"), JSON.stringify(stats, null, 2));
console.log("\nstats -> aq5-stats.json");
