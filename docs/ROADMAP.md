...
Bug latent corrigé (`fd4538d`) : le champ texte d'un paramètre `unit: "percent"`
affichait « 50 % » mais lisait la frappe dans les bornes FRACTION du curseur —
taper « 60 » s'écrêtait à 1 (= 100 %), champ inutilisable sur les 186 paramètres
percent du registre. `parseEffectParamValue` (`ParamPanel.tsx`), lecture inverse
de `formatEffectParamValue` par unité, généralise le mécanisme de la voie B ;
deux stories le gardent (`PercentInputConvertsToFraction`,
`PercentInputClampsInDisplaySpace`).
