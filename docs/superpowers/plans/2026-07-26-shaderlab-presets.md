...
**Tech Stack:** React 19 + TypeScript, Vitest (Node environment — no test renders a React component, this project's fixed convention), Tauri v2 commands (`rfd` for native dialogs, never `@tauri-apps/plugin-dialog`), `serde_json` (already a Rust dependency, unused until now).
...
- Produces (consumed by Task 2's `presetStore.ts`, Task 3's `usePresets.save`, Task 4's `usePresets.applyTo`, Task 5's `presetsDiffer`, Task 6's import validation):
  ...
  export interface SkipNotice { reason: "missing-effect" | "photo-layer"; effectId?: string; }
...
describe("capture", () => {
  ...
  it("excludes a photo layer (imageSource present) and reports a photo-layer SkipNotice", () => {
    ...
    expect(skipped).toEqual([{ reason: "photo-layer" }]);
  });
});
...
export function capture(layers: LayerState[], name: string): { preset: PresetDocument; skipped: SkipNotice[] } {
  ...
  for (const layer of layers) {
    if (layer.imageSource) {
      skipped.push({ reason: "photo-layer" });
      continue;
    }
    presetLayers.push({
...
- [ ] **Step 1: Add the 4 Family-A Rust commands (`presets_dir`, `is_safe_preset_id`, `list_preset_ids`, `read_preset`, `write_preset`, `delete_preset`)**
...
**Interfaces:**
- Consumes: `PresetStore`/`TauriPresetStore` (Task 2), `capture` (Task 1), `useContextualPanel` (existing, `src/ui/contextualPanel.ts:36-48`), `PanelRail`/`PanelRailItem` (existing, `src/components/dockedPanel/PanelRail.tsx`), `DockedPanelSpec` (existing, `src/components/dockedPanel/PanelColumn.tsx:8-14`).
- Produces (this task's partial surface; Task 4/5 extend the same hook and file, never duplicate them):
  ```ts
  export function usePresets(store: PresetStore): {
    summaries: PresetSummary[];
    refresh: () => Promise<void>;
    save: (name: string) => Promise<void>;
    // applyTo/rename/remove/exportPreset/importPreset/activePresetId/isDirty/
    // updateActive/copyActiveAsNew added in Task 4/5/6 — NOT stubbed here,
    // added when their task lands (no placeholder methods).
  }
...
export function usePresets(store: PresetStore) {
   ...
   *  name confirmation (design.md §5.3) is the CALLER's responsibility
   *  (Task 4/App.tsx wires the Dialog) — this function always creates. */
...
export function PresetPanel({ summaries, hasLayers, onSave }: PresetPanelProps) {
        ...
        <ul className="preset-panel__list">
          {summaries.map((summary) => (
            <li key={summary.id} className="preset-panel__row">
              <span className="preset-panel__row-name">{summary.name}</span>
            </li>
          ))}
        </ul>
      )}
...
(Task 4 adds the "apply on click" handler and Task 5's dirty banner to this same file — the `<li>` becomes a button and gains a rename-on-double-click affordance there, per design.md §5.3, not invented ahead of time here.)
...
Before writing this CSS, grep `src/design/{primitives,semantic,components}.css` for the exact token names (`--input-background`, `--border`, `--radius-control`, `--foreground`, `--text-secondary`, `--text-primary`, `--font-size-sm`, `--space-1/2/3`) — copy real names verbatim (`Dialog.tsx`'s CSS, `dialog.css`, already read in this plan, confirms `--text-primary`/`--text-secondary`/`--surface-panel`/`--space-*` exist; `ColorGroupControl`'s Tailwind classes confirm `--radius-control`/`border-border`/`text-foreground`/`text-muted-foreground` exist as Tailwind-mapped tokens — reconcile the two naming families actually used by sibling files in `src/components/` before finalizing, `npm run lint:tokens` in Step 4 catches any mismatch).

- [ ] **Step 3: Wire `App.tsx`**

Add imports near the other feature imports (after the `ColorPickerPanel`/`EffectParam` imports at line 44-45):
...
Add the store instance and hook, near the other `useRef`/hook calls (after `sessionRef`, line 65):
...
Add the 4th contextual panel, alongside the 3 existing ones (line 668-670):
...
Update `isPanelVisible` (line 672-675) to cover `"presets"`:

```ts
  const isPanelVisible = useCallback(
    (id: string) =>
      id === "presets" ? presetsPanel.visible : id === "layers" ? layersPanel.visible : id === "params" ? paramsPanel.visible : maskPanel.visible,
    [presetsPanel.visible, layersPanel.visible, paramsPanel.visible, maskPanel.visible]
  );
```

Change the initial `dockLayout` (line 109) to lead with `"presets"`:

```ts
  const [dockLayout, setDockLayout] = useState<DockLayout>([["presets", "layers", "params", "mask"]]);
```

Add a `presetsFolded` state alongside `layersFolded`/`paramsFolded`/`maskFolded` (line 94-96):

```ts
  const [presetsFolded, setPresetsFolded] = useState(false);
```

**Blocking confirmation on a photo-layer exclusion at capture** (design doc §9, decided by Antoine 2026-07-26 — not the simple-warning default originally proposed): `capture()` is pure and cheap, so `App.tsx` calls it once up front to inspect `skipped` BEFORE ever writing to disk. If any entry has `reason === "photo-layer"`, a blocking `Dialog` lists the count and gets explicit confirmation; only on confirm does the actual `presets.save` (which re-captures internally, deterministically identical) run. Cancel writes nothing.

Add state for the pending save, alongside `presetsFolded`:

```ts
  const [pendingPhotoLayerSave, setPendingPhotoLayerSave] = useState<{ name: string; excludedCount: number } | null>(null);
```

Add the gating function, near the other `handle*` functions:

```ts
  function requestSavePreset(name: string) {
    const { skipped } = capture(sessionRef.current.layers(), name);
    const excludedCount = skipped.filter((s) => s.reason === "photo-layer").length;
    if (excludedCount > 0) {
      setPendingPhotoLayerSave({ name, excludedCount });
      return;
    }
    presets.save(sessionRef.current.layers(), name);
  }
```

Add the "presets" entry as the FIRST item of the `panels` array (before `"layers"`, inside the `<PanelColumn panels={[...]}>` block, line 802), wired to `requestSavePreset` rather than calling `presets.save` directly:
...
Render the blocking confirmation `Dialog`, alongside the `PanelColumn`/`PanelRail` block (inside `<main className="workspace">`, after `PanelRail`):

```tsx
        <Dialog
          open={pendingPhotoLayerSave !== null}
          title="Calque photo exclu du preset"
          description={
            pendingPhotoLayerSave
              ? `${pendingPhotoLayerSave.excludedCount} calque${pendingPhotoLayerSave.excludedCount > 1 ? "s" : ""} de photo (double exposure) ne ${pendingPhotoLayerSave.excludedCount > 1 ? "seront" : "sera"} pas inclus dans le preset "${pendingPhotoLayerSave.name}" — une source de photo n'a de sens que dans ce document.`
              : undefined
          }
          ...
          actions={
            <>
              ...
              <Button
                variant="default"
                onClick={() => {
                  if (pendingPhotoLayerSave) presets.save(sessionRef.current.layers(), pendingPhotoLayerSave.name);
...
Add the "presets" rail item as the FIRST item of `PanelRail`'s `items` array (line 869):
...
Expected: no new violations beyond the 2 pre-existing `EmptyWorkspace.stories.tsx` findings — fix any token-name mismatch found in `PresetPanel.css` by using the project's real token name, never a new hardcoded literal.

- [ ] **Step 7: Commit**

```bash
git add src/hooks/usePresets.ts src/components/PresetPanel.tsx src/components/PresetPanel.css src/App.tsx
git commit -m "feat(app): add Presets docked card (save) wired into contextual panels" -- src/hooks/usePresets.ts src/components/PresetPanel.tsx src/components/PresetPanel.css src/App.tsx
```
...
```ts
  ...
  return { summaries, refresh, save, applyTo };
```
...
Replace the `<li>` row with a clickable row and thread an `onApply` prop:

```tsx
// src/components/PresetPanel.tsx — changes only
export interface PresetPanelProps {
  summaries: PresetSummary[];
  hasLayers: boolean;
  onSave: (name: string) => void;
...
export function PresetPanel({ summaries, hasLayers, onSave, onApply }: PresetPanelProps) {
  // ...unchanged save-row...
  return (
    <div className="preset-panel">
      {/* ...unchanged save-row... */}
      {summaries.length === 0 ? (
        ...
        <ul className="preset-panel__list">
          {summaries.map((summary) => (
            <li key={summary.id} className="preset-panel__row">
              <button type="button" className="preset-panel__row-name" onClick={() => onApply(summary.id)}>
                {summary.name}
              </button>
            </li>
          ))}
...
(Confirm `--surface-hover` is the project's real hover-surface token name before writing this — `npm run lint:tokens` in Step 6 catches a mismatch.)

- [ ] **Step 3: Wire the confirmation Dialog in `App.tsx`**

Add state for the pending apply, near `colorPicker` (line 98-108):

```ts
  const [pendingPresetApply, setPendingPresetApply] = useState<string | null>(null);
```

Add the apply handler, near the other `handle*` functions:
  ...
  function applyPreset(id: string) {
    presets.applyTo(
      ...
      (newLayers) => {
        ...
        setLayers(sessionRef.current.layers());
...
Wire `PresetPanel`'s `onApply` to `requestApplyPreset` (in the `panels` array's `"presets"` entry, Task 3's block):

```tsx
              content: <PresetPanel
                  summaries={presets.summaries}
                  hasLayers={layers.length > 0}
                  onSave={(name) => presets.save(sessionRef.current.layers(), name)}
...
3. Confirm → stack is replaced; Ctrl+Z undoes back to the previous stack (undo works because `applyPreset` goes through `sessionRef.current.commit`, same path as every other mutation).
...
```ts
  ...
  return { summaries, refresh, save, applyTo, isDirtyOf, activePresetId: active?.id ?? null, updateActive, copyActiveAsNew };
```

Selecting a different layer, adding/removing a layer, or applying a DIFFERENT preset must clear `active` (design.md §5.4: "Sélectionner un autre calque, ajouter/retirer un calque, ou appliquer un autre preset efface `activePresetId`"). `usePresets` cannot see layer-selection changes on its own (it only knows what `App.tsx` passes into `applyTo`/`updateActive`) — `App.tsx` (Step 6) is responsible for calling a new `clearActive` when those events happen. Add it:
...
Compute `isDirty` from the hook, and clear `active` on the events design.md §5.4 lists. Add near `presets` (right after `const presets = usePresets(...)`):

```ts
  const presetIsDirty = presets.isDirtyOf(layers);
```

In `handleAdd`/`handleRemove` (existing layer-mutation handlers) and in `selectLayer` (existing selection handler), add a call to `presets.clearActive()` — locate each function by name in `App.tsx` and add the call as its first line, e.g.:
...
(Adjust to each function's actual existing body — the only required change is prepending `presets.clearActive();`, nothing else about their logic changes. `handleAdd`/`handleRemove` similarly gain this as their first line.)
...
```ts
  function applyPreset(id: string) {
    ...
    presets.applyTo(/* ...unchanged... */);
  }
```

Render the banner inside the `"presets"` panel content, below `PresetPanel` (in the `panels` array's `"presets"` entry):
                ...
                <>
                  ...
                  {presetIsDirty && presets.activePresetId && (
                    <div className="preset-panel__dirty-banner">
                      ...
                      <Button size="sm" variant="secondary" onClick={() => presets.updateActive(sessionRef.current.layers())}>
                        Mettre à jour
                      </Button>
          ...
          actions={
            <>
              ...
              <Button
                ...
                onClick={() => {
                  if (pendingPresetCopyName) presets.copyActiveAsNew(sessionRef.current.layers(), pendingPresetCopyName.trim());
...
.preset-panel__dirty-banner {
  ...
  gap: var(--space-2);
  padding: var(--space-2);
  background: var(--surface-warning, var(--surface-panel));
  border-radius: var(--radius-control);
  font-size: var(--font-size-sm);
}
```

(Confirm whether the project already has a `--surface-warning`-family token before using it — grep `src/design/semantic.css` for `warning`; if absent, use the closest existing semantic surface token instead of inventing one, per Global Constraints.)
...
2. Click "Mettre à jour" → the preset's saved file (on disk) now matches the current stack; banner disappears.
3. Apply a preset, tweak a param, click "Créer une copie", type a name, confirm → a NEW preset entry appears (original unchanged); banner disappears (the new copy becomes `activePresetId`).
4. Select a different layer, or add/remove a layer, without touching params → banner disappears (activePresetId cleared) even though nothing was "fixed" — matches design.md §5.4's explicit list of events that forget the active preset.
...
```tsx
// src/components/PresetPanel.tsx — additional props/rows
export interface PresetPanelProps {
  summaries: PresetSummary[];
  hasLayers: boolean;
  onSave: (name: string) => void;
...
Add an "Importer" button near the save row, and an inline "Exporter" affordance per row:
...
```tsx
      ...
      {/* ...list unchanged except each row gains an export button... */}
          {summaries.map((summary) => (
            <li key={summary.id} className="preset-panel__row">
              <button type="button" className="preset-panel__row-name" onClick={() => onApply(summary.id)}>
                {summary.name}
...
Wire the two new `PresetPanel` props:
...
## Self-review (performed before commit, per mission instructions)

**1. PRD § Presets coverage** — every bullet in `PRD.md:55-89` has a task:
- Capture without masks → T1 (`capture`).
- Confirmation on non-empty stack apply → T4.
- Confirmation on save-under-existing-name → **not built as a separate confirmation**: T3's `save` always creates a NEW preset (design.md §5.1 doesn't distinguish "save" from "save as new"; PRD.md:63-65 asks for it explicitly). **Gap found and left open below**, not silently dropped.
- Rename by double-click → **not built**: `PresetStore.rename` exists (T2) but no `App.tsx`/`PresetPanel` UI calls it. **Gap found and left open below.**
- Update vs copy after param drift → T5.
...
- Photo-layer exclusion at capture requires a BLOCKING confirmation, not a silent warning (design doc §9, decided by Antoine 2026-07-26, PRD.md:129-131's "no silent loss" rule) → T3 (`pendingPhotoLayerSave` + `Dialog`, gated by `requestSavePreset` calling pure `capture()` before any write).
...
**Gaps the design doc still leaves insufficiently specified to write a task without guessing** (flagged rather than invented — narrower than before this revision, since Antoine's 2026-07-26 decisions closed the photo-layer-confirmation and import-dedup points that were previously open here):
- **Overwrite-by-name confirmation** (PRD.md:63-65, design.md §5.3): design.md says "`presetStore.list()` avant `save()` détecte une collision de nom → même composant de confirmation que 5.2" but never specifies WHERE this check runs (inside `usePresets.save`? in `App.tsx` before calling it?) or what the Dialog's exact copy is. T3's `save`/`requestSavePreset` as written gate ONLY on the photo-layer exclusion (Antoine's decision above) — a save under an already-taken NAME still always creates a fresh preset today, with no name-collision check at all. This is a real, still-open product gap, distinct from the photo-layer confirmation (which IS built in T3). Needs a follow-up task once design.md is amended to specify the check's location and the confirmation's exact wording.
- **Rename by double-click** (PRD.md:64-65, design.md §5.3): the storage-layer `rename()` (T2) exists and is tested, but no task wires a double-click-to-edit affordance in `PresetPanel`'s row. design.md doesn't specify the inline-edit interaction (contentEditable? a text input swapped in? commit on blur or Enter only?) precisely enough to write steps without guessing UI behavior design.md never described. Needs a follow-up task once that interaction is specified.

Both remaining gaps should go back to Antoine before implementation starts on this plan, or be accepted as an explicitly deferred follow-up plan — they are NOT folded into T3-T6 above by guessing, per the mission's "ne devine aucune API" instruction.
...
- `PresetLayer`/`PresetDocument`/`SkipNotice`/`ApplyWarning` (T1) used identically in T2 (`PresetStore`), T4 (`apply`/`applyTo`), T5 (`toPresetLayers`/`presetsDiffer`), T6 (`validatePresetDocument`).
- `usePresets`'s return shape grows monotonically: T3 `{summaries, refresh, save}` → T4 adds `applyTo` → T5 adds `isDirtyOf, activePresetId, updateActive, copyActiveAsNew, clearActive` → T6 touches nothing on the hook (only `PresetPanel`/`App.tsx`/`presetStore.ts`). No task removes or renames a field a later task depends on.
