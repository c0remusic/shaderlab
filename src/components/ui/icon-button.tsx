...
export function IconButton({ label, tooltip = label, size = "default", variant = "quiet", type = "button", className, children, ...props }: IconButtonProps) {
  const button = (
    <ButtonPrimitive
      ...
      aria-label={label}
      className={cn(
        "inline-flex shrink-0 items-center justify-center rounded-[var(--radius-control)] border border-transparent bg-transparent text-muted-foreground transition-colors hover:bg-muted hover:text-foreground active:bg-[var(--surface-active)] focus-visible:outline-[length:var(--focus-width)] focus-visible:outline-solid focus-visible:outline-[var(--focus-color)] focus-visible:outline-offset-[var(--focus-offset)] disabled:cursor-not-allowed disabled:text-[var(--text-disabled)]",
