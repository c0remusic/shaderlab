...
export function usePhotoLayer({
  ...
  /* `importTextureFromPath` vivait ici — ajouter un scan de matière comme
   * CALQUE PHOTO, avec sa transform de couverture et son mode `soft-light`.
   * RETIRÉ le 2026-08-19 avec la carte « Textures » du dock, son unique
   * appelant : depuis ADR-0018 un effet échantillonne la bibliothèque
   * lui-même (binding 7), et poser la même image en calque photo faisait
   * double emploi. Le geste n'est pas perdu pour autant — un scan reste un
   * fichier image, donc `importPhotoFromPath` et le bouton d'import de la
   * Toolbar l'ouvrent comme n'importe quelle photo. Ce qui part est la
   * GRILLE de vignettes qui en faisait un geste à part. */
