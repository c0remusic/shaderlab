...
fn halftone_dot(
  ...
  let rayon = size * 0.5 * scale * sqrt(clamp(ink, 0.0, 1.0));
...
fn fs_main(uv: vec2<f32>, color: vec4<f32>) -> vec4<f32> {
  ...
  if (mode == ${MODE_CMYK}) {
    ...
    let dC = halftone_dot(px, centerPx, rot + ${SCREEN_ANGLES[0]}.0 * deg, size, scale, softness, 0, blackPoint, whitePoint, dims);
    let dM = halftone_dot(px, centerPx, rot + ${SCREEN_ANGLES[1]}.0 * deg, size, scale, softness, 1, blackPoint, whitePoint, dims);
    let dY = halftone_dot(px, centerPx, rot + ${SCREEN_ANGLES[2]}.0 * deg, size, scale, softness, 2, blackPoint, whitePoint, dims);
    let dK = halftone_dot(px, centerPx, rot + ${SCREEN_ANGLES[3]}.0 * deg, size, scale, softness, 3, blackPoint, whitePoint, dims);
  ...
  if (mode == 1) {
    ...
    let dR = halftone_dot(px, centerPx, rot + ${SCREEN_ANGLES[0]}.0 * deg, size, scale, softness, 0, blackPoint, whitePoint, dims);
    let dG = halftone_dot(px, centerPx, rot + ${SCREEN_ANGLES[1]}.0 * deg, size, scale, softness, 1, blackPoint, whitePoint, dims);
    let dB = halftone_dot(px, centerPx, rot + ${SCREEN_ANGLES[2]}.0 * deg, size, scale, softness, 2, blackPoint, whitePoint, dims);
