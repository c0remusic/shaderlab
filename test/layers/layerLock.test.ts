...
describe("setLayerLocked", () => {
  it("pose et retire le verrou, et rend false sur un no-op", () => {
    const stack = new LayerStack();
    const id = stack.addLayer("glow");
    expect(stack.layers[0].locks?.all).toBeUndefined(); // défaut : non verrouillé
    expect(stack.setLayerLock(id, "all", true)).toBe(true);
    expect(stack.layers[0].locks?.all).toBe(true);
    expect(stack.setLayerLock(id, "all", true)).toBe(false); // déjà verrouillé
    expect(stack.setLayerLock(id, "all", false)).toBe(true);
    expect(stack.layers[0].locks?.all ?? false).toBe(false);
  });

  it("rend false sur un id absent (pas d'entrée d'historique vide)", () => {
    const stack = new LayerStack();
    expect(stack.setLayerLock("fantome", "all", true)).toBe(false);
  });
  ...
  it("survit à clone() — le verrou fait partie de l'état d'historique", () => {
    ...
    expect(stack.clone().layers[0].locks?.all).toBe(true);
  });
});
...
describe("opérations REFUSÉES sur un calque verrouillé", () => {
  ...
  it("reorderLayer", () => {
    ...
    expect(stack.setLayerLock(a, "all", true)).toBe(true);
...
describe("opérations AUTORISÉES sur un calque verrouillé", () => {
  ...
  it("duplicateLayer reste possible, et la copie hérite du verrou", () => {
    ...
    expect(stack.layers[1].locks?.all).toBe(true);
  });
...
describe("un calque NON verrouillé n'est pas affecté", () => {
  it("les mêmes opérations passent sur son voisin libre", () => {
    ...
    expect(stack.setLayerLock(locked, "all", true)).toBe(true);
