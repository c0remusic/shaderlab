...
describe("compositeUvToPhotoUv", () => {
  ...
  it("identité (scale=1, rotation=0) : le centre composite tombe au centre photo", () => {
    const transform = { x: 500, y: 500, scaleX: 1, scaleY: 1, rotation: 0 };
  ...
  it("une échelle 2 double la taille apparente de la photo dans l'espace composite", () => {
    const transform = { x: 500, y: 500, scaleX: 2, scaleY: 2, rotation: 0 };
  ...
  it("une rotation de 90° échange les axes locaux", () => {
    const transform = { x: 500, y: 500, scaleX: 1, scaleY: 1, rotation: Math.PI / 2 };
...
describe("resetTransform / centerTransform / fitToCanvas", () => {
  ...
  it("resetTransform centre sur le fond, échelle 1, rotation 0", () => {
    expect(resetTransform(bgSize)).toEqual({ x: 500, y: 400, scaleX: 1, scaleY: 1, rotation: 0 });
  });

  it("centerTransform recentre sans toucher à l'échelle ni à la rotation", () => {
    const transform = { x: 10, y: 20, scaleX: 2.5, scaleY: 2.5, rotation: 1.1 };
    expect(centerTransform(transform, bgSize)).toEqual({ x: 500, y: 400, scaleX: 2.5, scaleY: 2.5, rotation: 1.1 });
  });

  it("fitToCanvas fait tenir une photo PLUS GRANDE que le fond (contain, pas cover)", () => {
    const transform = { x: 0, y: 0, scaleX: 3, scaleY: 3, rotation: 0 };
    // Photo 2000x1000, fond 1000x800 -> min(0.5, 0.8) = 0.5 (contain).
    const fitted = fitToCanvas(transform, bgSize, { width: 2000, height: 1000 });
    expect(fitted.scaleX).toBeCloseTo(0.5, 6);
  ...
  it("fitToCanvas agrandit une photo plus petite que le fond", () => {
    ...
    expect(fitToCanvas(transform, bgSize, { width: 500, height: 200 }).scaleX).toBeCloseTo(2, 6);
  });

  it("fitToCanvas conserve la rotation", () => {
    const transform = { x: 0, y: 0, scaleX: 1, scaleY: 1, rotation: 0.7 };
  ...
  it("fitToCanvas laisse l'échelle inchangée sur une photo de taille dégénérée", () => {
    const transform = { x: 3, y: 4, scaleX: 1.75, scaleY: 1.75, rotation: 0 };
...
describe("transformFromCornerDrag", () => {
  ...
  for (const index of CORNER_INDICES) {
    it(`le coin opposé au coin ${index} tiré reste immobile — sans rotation`, () => {
      const transform: LayerTransform = { x: 500, y: 500, scaleX: 1, scaleY: 1, rotation: 0 };
    ...
    it(`le coin opposé au coin ${index} tiré reste immobile — avec rotation`, () => {
      const transform: LayerTransform = { x: 420, y: 380, scaleX: 1.7, scaleY: 1.7, rotation: 0.63 };
      const before = cornerAt(transform, OPPOSITE[index]);
      const next = transformFromCornerDrag(transform, photoSize, alongDiagonal(transform, index, 1.4), index);
      const after = cornerAt(next, OPPOSITE[index]);
      expect(after.x).toBeCloseTo(before.x, 6);
    ...
    it(`le coin ${index} tiré suit le pointeur (le drag n'ancre pas le centre)`, () => {
      ...
      expect(next.scaleX).toBeCloseTo(transform.scaleX * 1.2, 6);
    });
  }
  ...
  it("réappliquer le même pointeur au résultat est stable (l'ancre ne dérive pas pendant le drag)", () => {
    ...
    expect(twice.scaleX).toBeCloseTo(once.scaleX, 6);
  ...
  it("un pointeur posé sur l'ancre clampe à MIN_TRANSFORM_SCALE sans déplacer l'ancre", () => {
    const transform: LayerTransform = { x: 500, y: 500, scaleX: 1, scaleY: 1, rotation: 0.3 };
    const anchor = cornerAt(transform, 0);
    const next = transformFromCornerDrag(transform, photoSize, anchor, 2);
    expect(next.scaleX).toBe(MIN_TRANSFORM_SCALE);
  ...
  it("l'ancrage « centre » (Alt) garde le centre fixe et grandit dans les 4 directions", () => {
    ...
    expect(doubled.scaleX).toBeCloseTo(2, 5);
    expect(doubled.x).toBe(500);
  });

  it("l'ancrage « centre » est clampé par MIN_TRANSFORM_SCALE quand le pointeur est sur le centre", () => {
    const transform: LayerTransform = { x: 500, y: 500, scaleX: 1, scaleY: 1, rotation: 0 };
    expect(transformFromCornerDrag(transform, photoSize, { x: 500, y: 500 }, 2, "center").scaleX).toBe(
      MIN_TRANSFORM_SCALE,
    );
  });

  it("une photo de taille dégénérée laisse la transform inchangée (jamais de NaN)", () => {
    const transform: LayerTransform = { x: 500, y: 500, scaleX: 1, scaleY: 1, rotation: 0.2 };
...
describe("échelle NON HOMOTHÉTIQUE — les deux axes bougent séparément", () => {
  const photoSize = { width: 400, height: 200 };

  it("un coin tiré change les DEUX axes indépendamment", () => {
    const transform: LayerTransform = { x: 500, y: 500, scaleX: 1, scaleY: 1, rotation: 0 };
    // Coin 2 (bas-droit) tiré : l'ancre est le coin haut-gauche, à (300, 400).
    // Pointeur à (1100, 600) => largeur 800 pour 400 natifs, hauteur 200 pour
    // 200 natifs. Les deux facteurs DIFFÈRENT, ce qu'aucun geste ne pouvait
    // produire avant.
    const next = transformFromCornerDrag(transform, photoSize, { x: 1100, y: 600 }, 2);
    expect(next.scaleX).toBeCloseTo(2, 6);
    expect(next.scaleY).toBeCloseTo(1, 6);
  });

  it("le coin opposé reste immobile quand les deux axes changent", () => {
    const transform: LayerTransform = { x: 500, y: 500, scaleX: 1, scaleY: 1, rotation: 0 };
    const anchorBefore = computeHandleGeometry(transform, photoSize).corners[0];
    const next = transformFromCornerDrag(transform, photoSize, { x: 1100, y: 900 }, 2);
    const anchorAfter = computeHandleGeometry(next, photoSize).corners[0];
    expect(anchorAfter.x).toBeCloseTo(anchorBefore.x, 6);
    expect(anchorAfter.y).toBeCloseTo(anchorBefore.y, 6);
  });

  it("Maj CONSERVE le rapport d'axes courant, il ne le RAMÈNE PAS à 1", () => {
    // Photo déjà étirée 2:1 en échelle. Contraindre doit garder 2:1, pas
    // redresser — sinon « contraindre » réparerait une déformation qu'on n'a
    // pas demandé de réparer, et agrandir une photo étirée serait impossible.
    const transform: LayerTransform = { x: 500, y: 500, scaleX: 2, scaleY: 1, rotation: 0 };
    const next = transformFromCornerDrag(transform, photoSize, { x: 1400, y: 900 }, 2, "oppositeCorner", true);
    expect(next.scaleY / next.scaleX).toBeCloseTo(0.5, 6);
  });

  it("une poignée de côté HORIZONTALE ne touche que scaleX", () => {
    const transform: LayerTransform = { x: 500, y: 500, scaleX: 1, scaleY: 1.4, rotation: 0 };
    const next = transformFromEdgeDrag(transform, photoSize, { x: 900, y: 777 }, 1);
    expect(next.scaleY).toBe(1.4);
    expect(next.scaleX).not.toBeCloseTo(1, 3);
    // L'ordonnée du pointeur est IGNORÉE : une poignée de côté qui suivrait
    // l'autre axe serait un coin déguisé.
    expect(transformFromEdgeDrag(transform, photoSize, { x: 900, y: 0 }, 1).scaleX).toBeCloseTo(next.scaleX, 6);
  });

  it("une poignée de côté VERTICALE ne touche que scaleY", () => {
    const transform: LayerTransform = { x: 500, y: 500, scaleX: 1.9, scaleY: 1, rotation: 0 };
    const next = transformFromEdgeDrag(transform, photoSize, { x: 123, y: 900 }, 2);
    expect(next.scaleX).toBe(1.9);
    expect(next.scaleY).not.toBeCloseTo(1, 3);
  });

  it("le côté opposé reste immobile pendant un drag de côté, rotation comprise", () => {
    const transform: LayerTransform = { x: 500, y: 500, scaleX: 1, scaleY: 1, rotation: 0.7 };
    const before = computeHandleGeometry(transform, photoSize).edges[3];
    const next = transformFromEdgeDrag(transform, photoSize, { x: 820, y: 640 }, 1);
    const after = computeHandleGeometry(next, photoSize).edges[3];
    expect(after.x).toBeCloseTo(before.x, 6);
    expect(after.y).toBeCloseTo(before.y, 6);
  });

  it("sur une photo TOURNÉE, la poignée droite étire la largeur de la PHOTO, pas celle de l'écran", () => {
    // À 90°, la largeur de la photo est verticale à l'écran. Un pointeur
    // déplacé VERTICALEMENT doit donc changer scaleX — c'est le test qui
    // distingue un axe local d'un axe écran.
    const transform: LayerTransform = { x: 500, y: 500, scaleX: 1, scaleY: 1, rotation: Math.PI / 2 };
    const next = transformFromEdgeDrag(transform, photoSize, { x: 500, y: 1100 }, 1);
    expect(next.scaleY).toBe(1);
    expect(next.scaleX).toBeGreaterThan(1);
  });

  it("réappliquer un drag de côté au résultat est stable (l'ancre ne dérive pas)", () => {
    const transform: LayerTransform = { x: 500, y: 500, scaleX: 1, scaleY: 1, rotation: 0.3 };
    const pointer = { x: 860, y: 700 };
    const once = transformFromEdgeDrag(transform, photoSize, pointer, 1);
    const twice = transformFromEdgeDrag(once, photoSize, pointer, 1);
    expect(twice.scaleX).toBeCloseTo(once.scaleX, 6);
    expect(twice.x).toBeCloseTo(once.x, 6);
    expect(twice.y).toBeCloseTo(once.y, 6);
  });

  it("fitToCanvas REDRESSE une photo étirée — c'est le seul geste qui la remet homothétique", () => {
    const transform: LayerTransform = { x: 0, y: 0, scaleX: 3, scaleY: 0.4, rotation: 0.2 };
    const fitted = fitToCanvas(transform, { width: 1000, height: 800 }, photoSize);
    expect(fitted.scaleX).toBe(fitted.scaleY);
